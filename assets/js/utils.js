...
function afterOpsV2FirstRender(task) {
  ...
  window.addEventListener('ops-v2:first-render', function() { schedule(1500); }, { once: true });
  window.setTimeout(function() { schedule(0); }, 12000);
}
...
var _shardBasesReady = new Promise(function(resolve) {
  afterOpsV2FirstRender(function() {
    fetch('data/vllm/ci/shard_bases.json?_=' + Math.floor(Date.now()/1000))
