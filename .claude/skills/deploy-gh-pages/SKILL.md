...
The deploy workflow runs `render.py` first (regenerates `docs/_data/projects.json` from `config/projects.yaml`), then copies everything into `_site/`:

```
scripts/render.py  →  docs/_data/projects.json  (includes depends_on, build_workflows)

_site/
├── index.html          ← from docs/
├── assets/css/         ← from docs/
├── assets/js/          ← from docs/
├── _data/projects.json ← from docs/ (MUST be regenerated by render.py)
└── data/               ← from data/ (project JSON files, including build_times.json)
```

**CRITICAL**: `projects.json` is generated by `render.py` from `config/projects.yaml`. If you skip render, fields like `depends_on` and `build_workflows` won't appear in the JSON, and the Builds tab will show all N/A.
